...
function generate_data(; n = 1000, rng = StableRNG(123))
    ...
    Y₁ = μY(ones(n), W) .+ ϵ
    Y₀ = μY(zeros(n), W) .+ ϵ
    return DataFrame(W = W, T = T, Tcat = categorical(T), Y = Y, Y₁ = Y₁, Y₀ = Y₀)
end
...
function plot_coverage(inference_method; n = 1000, K = 100)
    ...
    scatter!(
        ...
        estimates,
        color = :red,
        label = replace(string(inference_method), "_" => " "),
    )
...
function update_observables!(
    ...
    newestimates, newerrors, newmcestimates =
        repeat_inference(inference_method; n = n, K = K)
...
function make_animation(inference_method)
    ...
    fig, estimates, errors, mcestimates, title =
        plot_coverage(inference_method; n = 10_000, K = 100)
