...
function (estimator::FoldsCMRelevantFactorsEstimator)(
    acceleration::CPU1,
    estimand,
    dataset;
    cache = Dict(),
    ...
    for train_validation_indices in estimator.train_validation_indices
        η̂ = CMRelevantFactorsEstimator(train_validation_indices, estimator.models)
        η̂ₙ = η̂(
            estimand,
            dataset;
            cache = cache,
...
CMRelevantFactorsEstimator(; models, train_validation_indices = nothing) =
    CMRelevantFactorsEstimator(train_validation_indices, models)
...
CMRelevantFactorsEstimator(
    ...
    models,
    train_validation_indices = nothing,
) = CMRelevantFactorsEstimator(nothing, models)
...
function build_propensity_score_estimator(
    propensity_score,
    models,
    dataset;
    train_validation_indices = nothing,
...
function estimate_propensity_score(
    propensity_score,
    models,
    dataset;
    train_validation_indices = nothing,
...
function estimate_outcome_mean(
    outcome_mean,
    models,
    dataset;
    train_validation_indices = nothing,
    ...
    outcome_mean_estimator =
        ConditionalDistributionEstimator(outcome_model, train_validation_indices)
    return try_fit_ml_estimator(
        outcome_mean_estimator,
        outcome_mean,
        dataset;
...
function estimate_propensity_score_and_outcome_mean(
    ...
    propensity_score_estimate = estimate_propensity_score(
        propensity_score,
        models,
        dataset;
        train_validation_indices = train_validation_indices,
    ...
    outcome_mean_estimate = estimate_outcome_mean(
        outcome_mean,
        models,
        dataset;
        train_validation_indices = train_validation_indices,
...
function estimate_propensity_score_and_outcome_mean(
    ...
    propensity_score_estimate = @spawn estimate_propensity_score(
        propensity_score,
        models,
        dataset;
        train_validation_indices = train_validation_indices,
    ...
    outcome_mean_estimate = @spawn estimate_outcome_mean(
        outcome_mean,
        models,
        dataset;
        train_validation_indices = train_validation_indices,
...
function (estimator::CMRelevantFactorsEstimator)(
    estimand,
    dataset;
    cache = Dict(),
    verbosity = 1,
    ...
    propensity_score_estimate, outcome_mean_estimate =
        estimate_propensity_score_and_outcome_mean(
            acceleration,
            models,
            propensity_score,
            outcome_mean,
    ...
    # Build estimate
    estimate =
        MLCMRelevantFactors(estimand, outcome_mean_estimate, propensity_score_estimate)
    # Update cache
    update_cache!(cache, estimand, estimator, estimate)
...
function (estimator::CMBasedTMLE)(
    estimand,
    dataset;
    cache = Dict(),
    verbosity = 1,
    ...
    fluctuated_estimator = MLConditionalDistributionEstimator(
        fluctuation_model,
        estimator.train_validation_indices,
    )
    fluctuated_outcome_mean = try_fit_ml_estimator(
    ...
    # Build estimate
    estimate =
        MLCMRelevantFactors(estimand, fluctuated_outcome_mean, fluctuated_propensity_score)
...
function CMBasedFoldsTMLE(
    ...
    estimators = map(
        zip(initial_factors_estimate.estimates, train_validation_indices),
    ) do (η̂ₙ, fold_train_val_indices)
        fluctuation_model = Fluctuation(
            Ψ,
            η̂ₙ;
            tol = tol,
            max_iter = max_iter,
...
function (estimator::CMBasedFoldsTMLE)(
    acceleration::CPU1,
    estimand,
    dataset;
    cache = Dict(),
...
function (estimator::CMBasedFoldsTMLE)(
    ...
    @threads for estimator_index = 1:n_estimators
        fold_estimator = estimator.estimators[estimator_index]
        estimates[estimator_index] = fold_estimator(
            estimand,
            dataset;
            cache = cache,
            verbosity = verbosity,
