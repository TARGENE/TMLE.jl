...
"""
propensity_score(Ψ, collaborative_strategy::CollaborativeStrategy) =
...
"""
update!(strategy::CollaborativeStrategy, last_targeted_η̂ₙ, dataset) =
    error("Not Implemented Error.")
...
function step_k_best_candidate(
    ...
    use_fluct = false
    ps_iterator = StepKPropensityScoreIterator(
        collaborative_strategy,
        Ψ,
        dataset,
        models,
    ...
    ps_sequence = map(ps_iterator) do (g, ĝ)
        ...
        targeted_η̂ₙ, loss = get_new_targeted_candidate(
            last_targeted_η̂ₙ,
            ĝₙ,
            fluctuation_model,
            dataset;
...
function find_optimal_candidate(
    ...
    candidate_id = 1
    best_candidate = (
        targeted_η̂ₙ = last_candidate_info.targeted_η̂ₙ,
        cv_loss = last_candidate_info.cv_loss,
        id = candidate_id,
    )
    verbosity > 0 &&
        @info @sprintf("Initial candidate's CV loss: %.5f", best_candidate.cv_loss)
    while !exhausted(collaborative_strategy)
        candidate_id += 1
        last_targeted_η̂ₙ, last_loss, last_cv_targeted_η̂ₙ, last_cv_loss =
            last_candidate_info
        # Find best candidate
        ...
        # Evaluate candidate
        new_cv_targeted_η̂ₙ, new_cv_loss = evaluate_cv_candidate(
        ...
        last_candidate_info = (
            targeted_η̂ₙ = new_targeted_η̂ₙ,
            loss = new_loss,
            cv_candidate = new_cv_targeted_η̂ₙ,
            cv_loss = new_cv_loss,
        )

        # Update the best candidate or early stop
        if new_cv_loss < best_candidate.cv_loss
            verbosity > 0 && @info @sprintf(
                "New candidate's CV loss: %.5f, updating best candidate.",
                new_cv_loss
            )
            best_candidate =
                (targeted_η̂ₙ = new_targeted_η̂ₙ, cv_loss = new_cv_loss, id = candidate_id)
        elseif candidate_id - best_candidate.id > collaborative_strategy.patience
            verbosity > 0 && @info @sprintf(
                "New candidate's CV loss: %.5f, patience reached, terminating.",
                new_cv_loss
            )
            break
        else
            verbosity > 0 &&
                @info @sprintf("New candidate's CV loss: %.5f, continuing.", new_cv_loss)
        end
    end
    return best_candidate
end

function get_initial_candidate(
    η,
    fluctuation_model,
    dataset;
    verbosity = 1,
    ...
    targeted_η̂ = CMBasedTMLE(fluctuation_model, nothing)
    targeted_η̂ₙ = targeted_η̂(
        η,
        dataset;
        cache = cache,
        verbosity = verbosity,
...
function get_new_targeted_candidate(
    last_targeted_η̂ₙ,
    ...
    new_η = TMLE.CMRelevantFactors(
        last_targeted_η̂ₙ.estimand.outcome_mean,
        new_propensity_score_estimate.estimand,
    )
    # Define new nuisance factors estimate
    η̂ₙ = TMLE.MLCMRelevantFactors(
        new_η,
        use_fluct ? last_targeted_η̂ₙ.outcome_mean :
        last_targeted_η̂ₙ.outcome_mean.machine.model.initial_factors.outcome_mean,
        new_propensity_score_estimate,
    )
    # Fluctuate
    new_fluctuation = Fluctuation(
        fluctuation_model.Ψ,
        η̂ₙ;
...
function get_initial_cv_candidate(
    η,
    dataset,
    fluctuation_model,
    train_validation_indices,
    ...
    validation_loss =
        compute_validation_loss(targeted_η̂ₙ, dataset, train_validation_indices)
...
function compute_validation_loss(targeted_η̂ₙ, dataset, train_validation_indices)
    folds_val_losses = map(
        zip(train_validation_indices, targeted_η̂ₙ),
    ) do ((_, val_indices), fold_targeted_η̂ₙ)
...
function update_cv_folds_info!(
    ...
    folds_val_losses[fold_index] =
        mean_loss(targeted_η̂ₙ_train, selectrows(dataset, fold_train_val_indices[2]))
...
function fill_cv_folds_info!(
    ...
    nfolds = length(folds_targeted_η̂ₙ)
    @threads for fold_index = 1:nfolds
        update_cv_folds_info!(folds_targeted_η̂ₙ, args..., fold_index; kwargs...)
    end
end

function evaluate_cv_candidate(
    last_cv_targeted_η̂ₙ,
    fluctuation_model,
    propensity_score,
    models,
...
mean_loss(η̂ₙ, dataset) = mean(compute_loss(η̂ₙ.outcome_mean, dataset))

adapt_and_getloss(ŷ::Vector{<:Real}) = ŷ, RootMeanSquaredError()

adapt_and_getloss(ŷ::Vector{<:Distribution{Univariate,Distributions.Continuous}}) =
    TMLE.expected_value(ŷ), RootMeanSquaredError()
