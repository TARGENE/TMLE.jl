...
Fluctuation(Ψ, initial_factors; tol=nothing, max_iter=1, ps_lowerbound=1e-8, weighted=false, cache=false, prevalence=nothing) =
    Fluctuation(Ψ, initial_factors, tol, max_iter, ps_lowerbound, weighted, cache, prevalence)
...
function initialize_observed_cache(model, X, y)
    ...
    w .*= get_weights_from_prevalence(model.prevalence, y)
...
"""
function initialize_counterfactual_cache(model, X, y)
    Q⁰ = model.initial_factors.outcome_mean
    G⁰ = model.initial_factors.propensity_score
    Ψ = model.Ψ

    prevalence_weights = get_weights_from_prevalence(model.prevalence, y)
    ...
    for (vals, sign) in indicator_fns(Ψ)
        ...
        # w_ct is ununsed, passing prevalence_weights for downstream functions instead
...
function compute_counterfactual_aggregate!(counterfactual_cache, Q)
    ...
    for (idx, (ct_ŷ, sign, ct_covariates, weights)) in enumerate(zip(
        ...
        ct_aggregate .+= sign .* (weights .* expected_value(new_ct_ŷ))
...
function compute_gradient_and_estimate_from_caches!(
    ...
    Ψ̂ = plugin_estimate(ct_aggregate, counterfactual_cache.weights)
    gradient = ∇YX(observed_cache[:H], observed_cache[:y], Ey, observed_cache[:w]) .+ ∇W(ct_aggregate, Ψ̂, counterfactual_cache.weights)
    return gradient, Ψ̂
end
...
function MLJBase.fit(model::Fluctuation, verbosity, X, y)
    ...
    counterfactual_cache = initialize_counterfactual_cache(model, X, y)
