...
causal_estimands = [
    ...
    ATE(outcome = :Y₁, treatment_values = (T₁ = (case = 2, control = 0),)),
    CM(
        outcome = :Y₁,
        treatment_values = (T₁ = (case = 2, control = 0), T₂ = (case = 1, control = 0)),
    ),
    CM(outcome = :Y₁, treatment_values = (T₂ = (case = 1, control = 0),)),
    AIE(
        outcome = :Y₁,
        treatment_values = (T₁ = (case = 2, control = 0), T₂ = (case = 1, control = 0)),
    ),
...
@testset "Test misc" begin
    ...
    @test Set(groups[((:T₁, :W₁, :W₂),)]) == Set([
        statistical_estimands[1],
        statistical_estimands[2],
        statistical_estimands[6],
    ])
    @test Set(groups[((:T₁, :T₂, :W₁, :W₂), (:T₂, :W₂))]) ==
          Set([statistical_estimands[3], statistical_estimands[5]])
...
@testset "Test ordering strategies" begin
    ...
    optimal_ordering =
        @test_logs (:info, "Lower bound reached, stopping.") brute_force_ordering(
    ...
    ordering_from_groups_with_brute_force =
        groups_ordering(bad_ordering, brute_force = true)
    @test TMLE.evaluate_proxy_costs(ordering_from_groups_with_brute_force, η_counts) ==
          (3, 11)
end

@testset "Test ordering strategies with Joint Estimands" begin
    ATE₁ = ATE(outcome = :Y₁, treatment_values = (T₁ = (case = 1, control = 0),))
    ATE₂ = ATE(outcome = :Y₁, treatment_values = (T₁ = (case = 2, control = 1),))
