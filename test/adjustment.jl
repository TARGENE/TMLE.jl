...
@testset "Test BackdoorAdjustment" begin
    scm =
        StaticSCM(outcomes = [:Y₁, :Y₂], treatments = [:T₁, :T₂], confounders = [:W₁, :W₂])
    causal_estimands = [
    ...
    statistical_estimands =
        [identify(method, estimand, scm) for estimand in causal_estimands]

    for (causal_estimand, statistical_estimand) in
        zip(causal_estimands, statistical_estimands)
    ...
    @test statistical_estimands[3].treatment_confounders ==
          Dict(:T₁ => (:W₁, :W₂), :T₂ => (:W₁, :W₂))
    @test statistical_estimands[4].treatment_confounders ==
          Dict(:T₁ => (:W₁, :W₂), :T₂ => (:W₁, :W₂))
end
...
@testset "Test TMLE.to_dict" begin
    ...
    @test adjustment_dict ==
          Dict(:outcome_extra_covariates => [:C], :type => "BackdoorAdjustment")
