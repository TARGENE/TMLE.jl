...
@testset "Test clever_covariate_and_weights: 1 treatment" begin
    ...
    propensity_score_estimator = TMLE.JointConditionalDistributionEstimator(
        Dict(:T => TMLE.MLConditionalDistributionEstimator(ConstantClassifier())),
    )
...
@testset "Test clever_covariate_and_weights: 2 treatments" begin
    ...
    propensity_score_estimator = TMLE.JointConditionalDistributionEstimator(
        Dict(:T₁ => distribution_estimator, :T₂ => distribution_estimator),
    )
    propensity_score_estimate = propensity_score_estimator(
...
@testset "Test compute_offset, clever_covariate_and_weights: 3 treatments" begin
    ...
    propensity_score_estimator = TMLE.JointConditionalDistributionEstimator(
        Dict(
            T => TMLE.MLConditionalDistributionEstimator(ConstantClassifier()) for
            T in (:T₁, :T₂, :T₃)
        ),
    )
    propensity_score_estimate = propensity_score_estimator(
