...
@testset "Test CMRelevantFactorsEstimator" begin
    ...
    # Estimator
    models = Dict(:Y => with_encoder(LinearRegressor()), :T₁ => LogisticClassifier())
    ...
    resampled_η̂ = TMLE.CMRelevantFactorsEstimator(
        models = models,
...
@testset "Test FitFailedError" begin
    ...
    models = Dict(:Y => with_encoder(LinearRegressor()), :T₁ => LinearRegressor())
    η̂ = TMLE.CMRelevantFactorsEstimator(models = models)
    try
    ...
    models =
        Dict(:Y => LogisticClassifier(), :T₁ => LogisticClassifier(fit_intercept = false))
    η̂ = TMLE.CMRelevantFactorsEstimator(models = models)
    try
        η̂(η, dataset; verbosity = 0)
        @test true === false
    ...
    pos_def_error_dataset =
        CSV.read(joinpath(DATADIR, "posdef_error_dataset.csv"), DataFrame)
    ...
    Q = TMLE.ConditionalDistribution(
        outcome,
