...
@testset "Test AdaptiveCorrelationOrdering Interface" begin
    ...
    @test g_init ==
          (TMLE.ConditionalDistribution(:T₁, (:T₂,)), TMLE.ConditionalDistribution(:T₂, ()))
    # Initialisation
    ...
    @test targeted_η̂.propensity_score.components[1].estimand ==
          TMLE.ConditionalDistribution(:T₁, (:T₂, :W₃))
    @test targeted_η̂.propensity_score.components[2].estimand ==
          TMLE.ConditionalDistribution(:T₂, (:W₃,))
end
...
@testset "Test Greedy Interface" begin
    ...
    train_validation_indices =
        MLJBase.train_test_pairs(resampling, 1:nrows(dataset), dataset, dataset.Y)
    initial_factors_estimator = TMLE.CMRelevantFactorsEstimator(
    ...
    targeted_η̂ₙ, loss = TMLE.get_initial_candidate(
        η,
        fluctuation_model,
        dataset;
        verbosity = verbosity,
    ...
    step_k_candidate_iterator = TMLE.StepKPropensityScoreIterator(
...
@testset "Integration Test using the AdaptiveCorrelationStrategy" begin
    ...
    Ψ = AIE(
        ...
        treatment_confounders = (T₁ = [:W₁, :W₂, :W₃], T₂ = [:W₁, :W₂, :W₃]),
    )
    # Define the estimator
    cache = Dict()
    resampling = StratifiedCV()
    ...
    @test ps_T2.machine.data[1] == DataFrame(INTERCEPT = fill(1.0, n_samples))
    ...
    cv_targeted_η̂ₙ, cv_loss = TMLE.get_initial_cv_candidate(
        η,
        dataset,
        fluctuation_model,
        train_validation_indices,
    ...
    validation_losses = []
    for (fold_estimate, (train_indices, val_indices)) in
        zip(cv_targeted_η̂ₙ, train_validation_indices)
        # Check the estimate is in the cache
        @test fold_estimate.outcome_mean.machine.model.initial_factors in
              values(cache[fold_estimate.estimand])
        # Check Fluctuation
        ...
        outcome_mean_estimate =
            fold_estimate.outcome_mean.machine.model.initial_factors.outcome_mean
        ...
        append!(
            validation_losses,
            TMLE.mean_loss(fold_estimate, selectrows(dataset, val_indices)),
        )
    end
    ...
    @test all(
        cde.train_validation_indices === nothing for cde in values(new_ĝ.cd_estimators)
    )
    ...
    outcome_mean_estimate_used =
        new_targeted_η̂ₙ.outcome_mean.machine.model.initial_factors.outcome_mean
    @test outcome_mean_estimate_used === targeted_η̂ₙ.outcome_mean
    ## The propensity score used for fluctuation is the new candidate
    fitted_propensity_score =
        new_targeted_η̂ₙ.outcome_mean.machine.model.initial_factors.propensity_score.estimand
    ...
    new_targeted_η̂ₙ_bis, new_loss_bis = TMLE.get_new_targeted_candidate(
        targeted_η̂ₙ,
        new_ĝₙ,
        fluctuation_model,
        dataset;
    ...
    outcome_mean_estimate_used =
        new_targeted_η̂ₙ_bis.outcome_mean.machine.model.initial_factors.outcome_mean
    @test outcome_mean_estimate_used ===
          targeted_η̂ₙ.outcome_mean.machine.model.initial_factors.outcome_mean
    ...
    new_cv_targeted_η̂ₙ, new_cv_loss = TMLE.evaluate_cv_candidate(
        cv_targeted_η̂ₙ,
        fluctuation_model,
        new_g,
        models,
    ...
    for (fold_id, fold_candidate) in enumerate(new_cv_targeted_η̂ₙ)
        ...
        @test fold_candidate.outcome_mean.machine.model.initial_factors.outcome_mean ===
              cv_targeted_η̂ₙ[fold_id].outcome_mean
    end
    @test TMLE.compute_validation_loss(
        new_cv_targeted_η̂ₙ,
        dataset,
        train_validation_indices,
    ) == new_cv_loss
    ...
    new_cv_targeted_η̂ₙ_bis, new_cv_loss_bis = TMLE.evaluate_cv_candidate(
        cv_targeted_η̂ₙ,
        fluctuation_model,
        new_g,
        models,
    ...
    for (fold_id, fold_candidate) in enumerate(new_cv_targeted_η̂ₙ_bis)
        ...
        @test fold_candidate.estimand.propensity_score === new_g
        @test fold_candidate.outcome_mean.machine.model.initial_factors.outcome_mean ===
              cv_targeted_η̂ₙ[fold_id].outcome_mean.machine.model.initial_factors.outcome_mean
    end
    @test TMLE.compute_validation_loss(
        new_cv_targeted_η̂ₙ_bis,
        dataset,
        train_validation_indices,
    ) == new_cv_loss_bis
    ...
    # Check the full loop
    candidate_info = (
        targeted_η̂ₙ = targeted_η̂ₙ,
        loss = loss,
        cv_targeted_η̂ₙ = cv_targeted_η̂ₙ,
        cv_loss = cv_loss,
        id = 1,
    )
