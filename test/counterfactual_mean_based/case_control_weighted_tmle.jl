...
@testset "CCW-OSE bootstrapping test" begin
    Random.seed!(42)
    Npop = 2_000_000
    # Simulate population
    W = rand(Bernoulli(0.5), Npop)
    ...
    # Standard OSE (no prevalence correction)
    ose_std = Ose()
    # CCW-OSE (with true prevalence)
    ...
    for i in 1:B
        sample = subsample_case_control(pop, n_sample, cc_prev, rng=Random.MersenneTwister(i))
        std_result, _ = ose_std(Ψ, sample; verbosity=0)
        ccw_result, _ = ose_ccw(Ψ, sample; verbosity=0)
        std_ose_results[i] = std_result.estimate
        ccw_ose_results[i] = ccw_result.estimate
        # Compare bias: CCW-OSE should be much less biased than standard OSE
        std_bias = abs(std_result.estimate - true_rd)
        ccw_bias = abs(ccw_result.estimate - true_rd)
        @test ccw_bias < std_bias / 2
        # Retrieve coverage
    ...
    # See if, on average, CCW-OSE outperforms standard OSE
