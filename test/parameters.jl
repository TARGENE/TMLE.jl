...
@testset "Test counterfactual_aggregate" begin
    ...
    η.G = machine(η_spec.G, (W=W,), dataset.T)
    fit!(η.G, verbosity=0)
    ...
    fit!(η.Q, verbosity=0)
    X₁ = (W=W, T=categorical(ones(Int, n), levels=levels(dataset.T)))
    X₀ = (W=W, T=categorical(zeros(Int, n), levels=levels(dataset.T)))
    ...
    var, val = fitted_params(η.Q).coefs[2]
    @test var == :T__0
    @test all(val ≈ -x for x ∈ cf_agg)
