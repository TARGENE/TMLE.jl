...
function check_estimands(reconstructed_estimands, estimands)
    for (index, estimand) in enumerate(estimands)
        ...
        for treatment in keys(estimand.treatment_values)
            @test estimand.treatment_values[treatment] ==
                  reconstructed_estimand.treatment_values[treatment]
            @test estimand.treatment_confounders[treatment] ==
                  reconstructed_estimand.treatment_confounders[treatment]
            @test estimand.outcome_extra_covariates ==
                  reconstructed_estimand.outcome_extra_covariates
        end
    end
end
...
@testset "Test Configurations" begin
    ...
    estimands = [
        AIE(
            ...
            treatment_confounders = (T1 = [:W11, :W12], T2 = [:W21, :W22]),
        ...
        ATE(;
            ...
            treatment_confounders = (T1 = [:W], T3 = [:W]),
        ),
        CM(;
            outcome = :Y3,
    ...
    configuration = Configuration(estimands = estimands)
...
@testset "Test with an SCM and causal estimands" begin
    ...
    configuration = Configuration(
        estimands = estimands,
        scm = StaticSCM(confounders = [:W], outcomes = [:Y1], treatments = [:T1, :T2]),
    ...
    for loaded_config in (config_from_yaml, config_from_json)
        ...
        @test identify(adjustment, estimand₁, scm) ==
              identify(configuration.adjustment, estimands[1], configuration.scm)
        # Estimand 2
